...
7. **Given** a phone in landscape, **When** the toy opens, **Then** every control is still visible and finger-sized, and the toolbar does not consume so much of the short screen height that the play area falls below the fill requirement in Acceptance Scenario 2 of User Story 1.
8. **Given** a laptop or desktop, **When** the toy opens, **Then** the toolbar looks and behaves as it does today.
...
- **FR-004**: The play field's shape MUST be allowed to differ from today's fixed landscape shape so that FR-001 can be satisfied in portrait. The toy MUST derive the play field's dimensions in cells from the drawing region's shape and size, subject to FR-005, FR-006, and FR-007. [NEEDS CLARIFICATION: Which adaptation strategy should the play field use? (a) Derive both the play field's shape *and* its resolution from the drawing region, so the play area always fills the screen with chunky, finger-friendly grains — this supersedes spec 001's fixed 270×160 / "250–300 cells across, never changing while the page is open" and its exact-preservation-on-rotation guarantee; (b) keep the fixed 270×160 play field and only drop the whole-number-cell-size restriction so it scales smoothly — simplest and preserves drawings exactly, but a portrait phone still gets a wide letterboxed strip covering roughly 40% of the screen with ~1.4-pixel grains, which does not satisfy FR-001/FR-002; (c) keep two fixed play fields, one landscape-shaped and one portrait-shaped, choosing between them once when the page loads — fills the screen on the orientation the child started in, letterboxes after a rotation. This spec is written assuming (a).]
...
- **FR-007**: The play field's total number of cells MUST NOT exceed the current default play field's cell count on any supported viewport, so the simulation can never become more expensive than it is today.
...
- **FR-020**: Every toolbar control MUST present a touch target of at least 44 screen pixels on each side, with enough separation that a fingertip cannot activate two at once. [NEEDS CLARIFICATION: How should the toolbar's thirteen controls fit a phone screen without eating the play area? (a) A compact always-visible bar: controls shrink toward the 44-pixel minimum and wrap to more than one row in portrait, becoming a narrow rail down one side in landscape where height is scarce — everything stays visible, nothing is hidden; (b) a translucent floating bar overlaying the play area, so the drawing surface is genuinely full-screen behind it — biggest play area, but the controls cover part of the drawing; (c) keep the current single wrapping row and let it take whatever height it needs — simplest, but on a narrow phone it wraps to roughly three rows and consumes about a third of the screen. This spec is written assuming (a).]
...
- **FR-026**: When the play field's dimensions in cells are re-derived — on an orientation change, or a viewport change large enough to warrant it — the drawing MUST be carried across as faithfully as the new shape allows: grains and objects keep their positions relative to the bottom-centre of the play field, so the ground stays at the bottom and a pile stays where the child put it; anything falling outside the new bounds is dropped; newly exposed area starts empty. [NEEDS CLARIFICATION: Is best-effort preservation across a re-derived play field acceptable? (a) Yes — carry the drawing across anchored to the bottom-centre, cropping what no longer fits; the child keeps her picture and the screen stays full, at the cost of spec 001's promise that a rotation preserves the drawing *exactly*; (b) no — the play field's cell dimensions are fixed for the life of the page, so a rotation letterboxes rather than re-shaping, honouring 001 exactly but leaving a rotated phone with a much smaller play area until reload; (c) the drawing may simply be cleared on a re-derivation — simplest, but a child losing her picture by turning the phone is a failure state in everything but name. This spec is written assuming (a).]
