...
- **Grass drinking the firebreak**: a stripe of water laid down to stop a burn is still ordinary water, so grass beside it drinks and grows into it under spec 007's rule, and a break left long enough beside thirsty grass can thin or open. Quenching itself never costs a drop (FR-017); only the grass does, and it stops once it is fully grown.
...
- **FR-002**: Star power MUST be transient: every star power cell MUST burn out on its own within its burn life, so no star power can remain on the field indefinitely. This applies to star power drawn where there is no grass as well — it fades after about a second like a sparkler rather than staying put. The ⭐ brush is an *action*, not a building material, so the toy's what-you-draw-stays-put promise does not apply to it; the toy's persistent sparkle-painter is the ✨ wand, and making star power permanent would duplicate the wand's role. Transience also bounds the worst case on a low-end tablet.
...
- **FR-009**: The glitter left by burned grass MUST be the toy's existing glitter grain from spec 005 — multicoloured rainbow sand in an already-glittered state — so it falls, tumbles, piles, twinkles, is erasable and clearable, and is caught by a rainbow exactly as spec 005's sprinkled glitter already is. This feature therefore adds exactly one new element type (star power) and no new physics for glitter. Burned grass MUST NOT leave a distinct new light-powder element: one treasure currency runs through the whole toy, the existing glitter's physics and tests come free, and the element set stays small as the constitution asks.
...
- **FR-017**: Extinguishing MUST NOT consume, move, or otherwise change the water cell that did it. One drop is therefore a permanent firebreak and water always wins — the simplest rule a non-reading child can discover, and one that never costs her anything to use. Quenching MUST NOT spend the water: a firebreak that burns away would be a step toward a losing state, and this toy has none.
- **FR-017a**: Spec 007's grass-drinking rule continues to apply unchanged to the water a child uses as a firebreak: grass beside it may drink it and grow, which can slowly open a gap in the break. This emergent interaction is deliberate and MUST NOT be special-cased away — the water is spent by the grass under spec 007's rules, never by the act of quenching (FR-017), and spec 007's rule that fully grown grass stops drinking still bounds it.
...
- **The ⭐ brush is a sparkler, not a paint** (FR-002). Confirmed in clarification: star power is an action the child performs, not a material she builds with, so it always fades after about a second even where there is no grass to catch. The toy's persistent sparkle-painter is already the ✨ wand, and a permanent star power would duplicate it. A bounded worst case on the Fire 7 falls out of this for free.
...
- **Glitter is the existing glitter grain** (FR-009). Reusing spec 005's multicoloured, already-glittered rainbow sand gives falling, piling, twinkling, erasing, and rainbow interaction for free, keeps the element set small as the constitution asks, and makes burned grass look like the toy's established treasure rather than a new material. Confirmed in clarification: one treasure currency across the whole toy, and no second powder element.
...
- **Water always wins, and is not used up** (FR-016, FR-017). This is the simplest rule a non-reading child can discover, and it makes the eraser and a puddle equally good firebreaks. Confirmed in clarification: quenching deliberately does not spend the water, because a firebreak that burns away would be a step toward a losing state. The one way a firebreak can still open is spec 007's grass drinking it (FR-017a), which is kept as a welcome emergent interaction rather than an exception.
- **The weather cycle is out of scope.** Fog, clouds, rain, steam, and evaporation are deliberately excluded and belong to the follow-up feature the issue describes (tracked as issue #21). This spec pins only the minimal quench rule.
