...
- [ ] T013 [US2] In `src/sim/scenes.ts`'s `loadScene`, replace the `'landscape2'` no-op stub with a call to `generateLandscape2(grid, objects)` after the unconditional clear (depends on T012; makes T009/T010/T011 pass)
...
- [ ] T015 [US2] Wire `onSelectScene` through `src/App.svelte`: add `function selectScene(id: SceneId) { playArea.loadScene(id); }` (importing `SceneId` from `./sim/types`) and pass it to `<Toolbar onSelectScene={selectScene} ...>`; do not call `selectTool`/`selectBrushSize` from this handler (FR-004) (depends on T004, T014)
