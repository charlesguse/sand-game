...
- **FR-007**: When an **unfuelled** star power cell (one the child drew, per spec 008) is extinguished by orthogonally adjacent water, exactly one of those adjacent water cells MUST be charmed into fog in the same simulation step. A **fuelled** star power cell — a blade of grass that caught — MUST be extinguished exactly as spec 008 requires and MUST leave the water untouched, so a puddle remains the permanent firebreak spec 008 promises. [NEEDS CLARIFICATION: should star power that arrived by burning grass also steam the water it reaches? Doing so makes a burning shoreline gorgeous but can mist a thin firebreak away and let a burn cross water, which spec 008's SC-007 forbids.]
...
- **The burn front does not steam water** (FR-007). Keeping fuelled star power's contact with water exactly as spec 008 wrote it preserves the promise that a pour of water is a permanent firebreak, which is the child's only control over a burn. Marked for clarification because the other reading is genuinely attractive to look at.
