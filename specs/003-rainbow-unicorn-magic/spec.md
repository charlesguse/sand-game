...
The child taps the 🌈 button, then taps somewhere in the play area. A rainbow appears right there and stays put. She switches back to 🩷 pink sand and pours a stream down onto the rainbow. The grains that touch the rainbow come out the other side changed: they are rainbow-colored now, and they keep falling and piling exactly the way sand always does, building a bright multi-colored heap on and around the rainbow. Water poured onto the rainbow gets turned into rainbow sand too — it stops flowing and starts piling. She can put down a couple of rainbows and run sand through all of them.
...
1. **Given** the 🌈 tool is selected, **When** the child taps a spot in the play area, **Then** one rainbow object appears centered on that spot and remains there until erased or cleared.
2. **Given** a rainbow is on screen, **When** a pink-sand grain moves into a cell touching the rainbow, **Then** that grain becomes rainbow sand.
3. **Given** a rainbow is on screen, **When** a water cell moves into a cell touching the rainbow, **Then** it becomes rainbow sand — it stops behaving like a liquid and starts falling and piling like a powder.
4. **Given** a magic-purple-dirt grain touches a rainbow, **When** the simulation advances, **Then** it becomes rainbow sand, exactly as pink sand does.
5. **Given** a grain that has already become rainbow sand, **When** it touches the same or another rainbow again, **Then** it stays rainbow sand and nothing else happens.
6. **Given** rainbow sand on screen, **When** the simulation advances, **Then** it falls, slides diagonally, piles with sloped sides, and sinks through water exactly as pink sand does.
7. **Given** sand pouring steadily onto a rainbow, **When** the simulation runs, **Then** the resulting pile is visibly multi-colored — the rainbow colors read as stripes or bands through the heap rather than one flat color.
8. **Given** two or more rainbows on screen, **When** sand falls through all of them, **Then** each one converts the grains that touch it, independently of the others.
9. **Given** a rainbow with no sand or water near it, **When** the simulation advances, **Then** nothing changes — a rainbow never creates grains on its own.
...
1. **Given** the 🦄 tool is selected, **When** the child taps a spot in the play area, **Then** one unicorn object appears at that spot and remains there until erased or cleared.
2. **Given** a unicorn is on screen, **When** a grain of any powder or a water cell moves into a cell touching the unicorn, **Then** a burst of ✨ sparkles and 💖 hearts appears around the unicorn.
3. **Given** a burst has appeared, **When** a moment passes, **Then** the particles drift upward and fade out completely, leaving no trace.
4. **Given** a unicorn that nothing is touching, **When** the child watches it without doing anything, **Then** it emits a few sparkles on its own every so often.
5. **Given** a heavy continuous stream of sand pouring onto a unicorn, **When** the simulation runs, **Then** the unicorn keeps celebrating but the number of particles on screen stays bounded and the toy stays smooth.
6. **Given** sparkles and hearts are floating over the play area, **When** they pass over sand, water, or another object, **Then** they change nothing — no grain is created, destroyed, moved, or recolored by a particle.
7. **Given** several unicorns are on screen, **When** sand touches each of them, **Then** each celebrates independently.
8. **Given** a unicorn buried under a heap of sand, **When** the simulation runs, **Then** nothing bad happens — the unicorn is still there, still reacts, and reappears when the sand is erased.
...
4. **Given** water poured onto an object, **When** the simulation advances, **Then** the water rests on top and spreads off the sides rather than passing through it.
5. **Given** an object, **When** any element moves anywhere in the play area, **Then** no cell inside the object's footprint is ever occupied by sand, dirt, or water.
6. **Given** the child places an object where sand or water already sits, **When** the object appears, **Then** the elements inside its footprint are simply cleared away to make room — the placement always succeeds and nothing is refused.
...
- **The object cap is already reached**: placing another one is never refused with an error or a message — see FR-005 for the agreed behavior at the cap.
...
- **Rapid repeated taps with an object tool**: each tap places at most one object and the object cap still holds; nothing stutters or piles hundreds of emoji on one spot.
...
- **FR-005**: The toy MUST allow more than one rainbow and more than one unicorn on screen at the same time, up to a fixed cap per type. [NEEDS CLARIFICATION: how many of each may exist at once, and what happens on the tap that exceeds the cap — silently replace the oldest object of that type, or silently ignore the tap? Either MUST be silent and non-punishing.]
...
- **FR-007**: Objects MUST NOT be affected by the simulation's gravity rules for elements. [NEEDS CLARIFICATION: when the child taps mid-air, does the object stay exactly where she tapped (a floating shelf, simplest and most predictable), or does it drop straight down until it rests on the ground or the top of a pile before locking in place?]
...
- **FR-012**: When an object is removed (by eraser or clear-all), the cells it occupied MUST become ordinary empty cells, and any elements resting on it MUST resume falling on the next simulation step.
...
- **FR-021**: The rainbow colors of rainbow sand MUST be assigned so that piles read as rainbow-striped. [NEEDS CLARIFICATION: is each grain's rainbow color fixed at the moment it is converted (cheapest, gives static striped piles), or does each grain's color cycle over time while it falls and then freeze when it comes to rest (a shimmer while falling), or does it keep cycling forever (most magical, most per-frame cost)? The frame-rate requirement in FR-030 takes precedence over the answer.]
...
- **FR-030**: The simulation MUST stay smooth (target 60fps, acceptable ≥30fps) at the default grid resolution on a mid-range laptop and a tablet, with the maximum number of objects on screen, particles at their cap, and the play area at least half full of a mixture of all elements.
...
- **FR-037**: The project MUST provide automated tests, runnable without a browser, covering at minimum: the rainbow conversion rule (an element in a rainbow's conversion zone becomes rainbow sand — FR-013 through FR-018), rainbow sand moving identically to pink sand (FR-019), and elements landing on and sliding off solid objects without entering their footprints (FR-009 through FR-011).
...
- **SC-012**: The toy renders at least 30 frames per second, targeting 60, on a mid-range laptop and a tablet in the worst observed case: maximum objects on screen, particles at their cap, and the play area at least half full of flowing elements.
