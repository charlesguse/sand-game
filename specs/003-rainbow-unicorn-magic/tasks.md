...
- [ ] T044 Line-interpolate `eraseObjectsInBrush` calls in `src/lib/PlayArea.svelte`'s `paintAt`, the same way `applyBrushLine` interpolates element painting between `lastGridPos` and `pos`, so a fast eraser drag whose consecutive pointer samples straddle an object's footprint cannot leave the object in place while erasing the elements around it per FR-031, SC-014 (partial)
- [ ] T045 Eliminate per-tick closure and array-spread allocations in the object/particle hot path — `applyRainbowConversions` and `isUnicornTouched` in `src/sim/objects.ts` each pass a fresh closure to `forEachZoneCell` per call, and `PlayArea.svelte`'s `render()` and the `eraseObjectsInBrush` call site build a new `[...rainbows, ...unicorns]` array every frame — to satisfy contracts/sim-core.md's "Allocates nothing" contract for `applyRainbowConversions`/`isUnicornTouched` and Constitution Principle IV's allocation-free hot loop guidance (contradicts)
