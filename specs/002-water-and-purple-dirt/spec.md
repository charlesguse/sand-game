...
8. **Given** a settled body of water on screen, **When** the child looks at it, **Then** individual cells show visibly different blue shades within a narrow blue range, so the water reads as lively rather than as a flat blue rectangle.
...
3. **Given** pink sand and purple dirt piled together, **When** the simulation runs, **Then** each grain keeps its own element and color — grains never change type or swap colors with each other.
4. **Given** a pile of purple dirt on screen, **When** the child looks at it, **Then** individual grains show visibly different purple shades within a narrow purple range, distinct from the pink range at a glance.
5. **Given** a grain of pink sand resting directly on a grain of purple dirt (or the reverse), **When** the simulation advances, **Then** neither sinks through the other; powders rest on powders.
...
- **FR-010**: Water left undisturbed in a container MUST settle to a flat surface [NEEDS CLARIFICATION: how complete must "finding its level" be? Sideways-only flow levels water within a single open container but cannot push water back *up* into the far arm of a U-shaped container or a tall narrow neck; a pressure-style rule that lets water rise makes U-tubes level correctly but is more work and more expensive per frame. Which behavior is required?].
...
- **FR-016**: Magic purple dirt piling MUST behave as [NEEDS CLARIFICATION: should purple dirt pile more steeply/stickily than pink sand, or behave identically to sand apart from color? The requester said steeper is nice-to-have and plain sand behavior in purple is acceptable; steeper piling means an extra distinct rule to specify, test, and eyeball].
...
- **FR-026**: Water MUST be rendered in a lively blue that shows visible variation across a body of water rather than reading as a single flat blue block. [NEEDS CLARIFICATION: is a fixed per-cell blue shade assigned at creation (cheapest, matches how sand grains vary) sufficient, or is an animated shimmer that changes over time required?]
...
- **Water is a simple cellular liquid, not a physics fluid**: no waves, no pressure visualization, no splashing droplets, no viscosity — the simplest rule that looks fun wins, per the project's simplicity principle.
