...
| Toolbar stays within the 44px floor at 28/30 controls, every representative viewport | `tests/unit/lib/layout.test.ts` (no file edit needed — it already derives counts from `shippedToolbarControls(...).length`) | Existing sweep re-runs automatically against the new counts once `toolbarControls.ts` lists the two new entries (SC-006) |
