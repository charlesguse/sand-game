...
| Emoji glyph coverage | newer emoji (Emoji 13.0+, e.g. 🪣 🪄) render fine; free to pick them | Segoe UI Emoji on Windows 10 / Fire draws them as an empty box — a control that needs one of these ships an inline SVG (e.g. `src/lib/BucketIcon.svelte`) instead of relying on the glyph |
